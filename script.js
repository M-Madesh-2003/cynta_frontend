...
function startVendorTracking(vendor) {
  if (navigator.geolocation) {
    navigator.geolocation.watchPosition(
      (position) => {
        const lat = position.coords.latitude;
        ...
        const updatedVendorLocation = { latitude: lat, longitude: lng };

        // Emit live vendor location to the backend
        socket.emit("vendorDetails", {
          ...vendor,
          location: updatedVendorLocation,
        });
